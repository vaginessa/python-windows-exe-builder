...
# DO NOT ENABLE IN CASE OF COMMAND-BASED APP 
...
if [ "$guichoice" == "yes" ] && [ "$sfilechoice" == "yes" ]; then 	pyinstaller --windowed --onefile ${pypath}
fi


# Case: User wants a GUI but doesn't need a single file

if [ "$guichoice" == "yes" ] && [ "$sfilechoice" == "no" ]; then 	pyinstaller --windowed ${pypath}
fi


# Case: User doesn't want GUI but wants a single file

if [ "$guichoice" == "no" ] && [ "$sfilechoice" == "yes" ]; then 	pyinstaller --onefile ${pypath}
fi


# Case: User doesn't want GUI or single file

if [ "$guichoice" == "no" ] && [ "$sfilechoice" = "no" ]; then 		pyinstaller ${pypath}
fi
